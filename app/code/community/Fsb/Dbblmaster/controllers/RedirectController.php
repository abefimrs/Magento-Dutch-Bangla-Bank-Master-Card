...
class Fsb_Dbblmaster_RedirectController extends Mage_Core_Controller_Front_Action {
	...
	/*	save cutomer redirect status */
        ...
        $order->addStatusToHistory($order->getStatus(), Mage::helper('dbblmaster')->__('Customer was redirected to DBBL Gateway.'));
        $order->save();
		
        $this->getResponse()
                ->setHeader('Content-type', 'text/html; charset=utf8')
                ->setBody($this->getLayout()
                ->createBlock('dbblmaster/redirect')
                ->toHtml());
    }
		...
		if(!empty($dbbl_transaction_id) && !empty($transaction_id)  ) 
		{
			...
			if($transaction_id)
			{
				...
				if($result_code=="000"){
					...
					$order->addStatusToHistory($order->getStatus(), Mage::helper('dbblmaster')->__('Customer successfully returned from DBBL'));
					...
					$order->addStatusToHistory($order->getStatus(), $transaction_status);
					$order->addStatusToHistory($order->getStatus(), $card_no);
					$order->addStatusToHistory($order->getStatus(), Mage::helper('dbblmaster')->__('Customer was rejected by DBBL'));
					$status = false;
